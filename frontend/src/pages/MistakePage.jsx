...
export default function MistakePage() {
  ...
  const [sidebarType, setSidebarType] = useState(null); // 'similar' or 'cards'
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      // 只加载错题
      const result = await fetchMistakeList({ ...filters, type: '错题' });
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('确认删除此题？')) return;
    try {
  ...
  return (
    <div className="mistake-page">
      <h2>❌ 错题本</h2>
      <p className="page-subtitle">记录你做错的题目，支持同类题练习和概念强化</p>
        ...
        <div className="mistake-list-view">
          ...
          {loading ? (
            ...
            <div className="list-empty">
              <p>📭 暂无错题</p>
              <p className="list-hint">在聊天或练习页面中点击「加入错题本」来添加</p>
            </div>
          ) : (
                ...
                <div key={item.id} className={`item-card ${selectedIds.has(item.id) ? 'selected' : ''}`}>
                  ...
                  <div className="item-body" onClick={() => setDetailItem(item)} style={{ cursor: 'pointer' }}>
                    <div className="item-header">
                      <span className="type-tag mistake">❌ 错题</span>
      ...
      {/* ── 统计摘要 ── */}
      {activeTab === 'summary' && <MistakeSummary />}
